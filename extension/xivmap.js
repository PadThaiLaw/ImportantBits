...
function xivmap(config) {
	 ...
	 */
	function makeRectangle(element, ratio, originalElement, color) {
	...
	function makeAccurateRectangle(element, ratio) {
		...
		if (rects.length) {
			for (var i = 0; i < rects.length; i++) {
				...
				html += makeRectangle(rect, ratio, element, color);
			}
			return html;
		}
		return makeRectangle(element, ratio, color);
	}
...
var padThai = document.createElement("div");
